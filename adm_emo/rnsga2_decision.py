...
from utils import compute_phi_decision_from_list
...
def main(
    ...
    for i in range(it_decision):
        ref_point = np.loadtxt(
            folder_path + "rp_decision_" + str(i) + "_" + PROB + "_" + str(OBJ),
            dtype=float,
        )
        list_ref_points.append(ref_point)
    ...
    FD = compute_phi_decision_from_list(
        archive_run, ideal, nadir, GENS, it_learning, it_decision, list_ref_points
    )
    print(FD)
...
def test():
    ...
    ap.add_argument(
        ...
        type=str,
        required=False,
        help="Normalization method (ever, front, no)",
    )
    ap.add_argument(
        "--epsilonValue", dest="eps", type=float, required=False, help="Size of the ROI"
    )
