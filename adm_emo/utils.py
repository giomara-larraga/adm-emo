...
def compute_phi_decision_from_list(
    objective_values_per_iteration,
    ideal,
    nadir,
    generations_per_iter,
    iterations_learning,
    iterations_decision,
    reference_points,
):
    ...
    for iteration in range(total_iterations):
        ...
        if iteration >= iterations_learning:
            max_gen_iter_data = objective_values_per_iteration[
                iteration_upper_bound - 1
            ]
            results_decision_positive.append(
                phi_learning.get_phi(
                    max_gen_iter_data, reference_points[iteration], nadir
                )[0]
            )

    phi_decision_phase_positive_nsga3 = phi_decision(
        iterations_decision, results_decision_positive, nadir
    )
    FD = phi_decision_phase_positive_nsga3.assess_decision_phase(
        set_of_RPs=np.asarray(decision_rp), main_RP=last_rp[0]
    )[0]
    return FD


def compute_phi_learning_from_list(
    objective_values_per_iteration,
    ideal,
    nadir,
    generations_per_iter,
    ...
    total_iterations = iterations_learning
    phi_learning = phi(ideal)
    ...
    for iteration in range(total_iterations):
        ...
        for gen in range(iteration_lower_bound, iteration_upper_bound):
            ...
            results_learning_positive.append(
                phi_learning.get_phi(data, reference_points[iteration], nadir)[0]
            )
