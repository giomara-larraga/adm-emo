...
import numpy as np
import pandas as pd
from Archive_EAs import RVEA_archive as RVEA
from problems import generate_dict_problems_dtlz, generate_dict_problems_wfg
from utils import compute_phi_from_list
...
def main(SEED, PROB, OBJ, GENS, CROS, CROS_PROB, CROS_REP, CROS_DIST, CROS_ALPHA, MUT, MUT_PROB, MUT_REPAIR, MUT_PMD, MUT_UMP, SEL, SEL_SIZE):
    problem_data = dict_problems[PROB+str(OBJ)]
    problem = problem_data["problem"]
    POP = len(problem_data["ref_dirs"])
        
    list_ref_points = []
    folder_path = "/projappl/project_2012477/implementation/adm-emo/adm_emo/reference_points/"
    it_learning = 4
    it_decision = 3
    for i in range(it_learning):
        ref_point = np.loadtxt(folder_path+"rp_learning_"+str(i)+"_"+PROB+"_"+str(OBJ), dtype=float) 
    ...
    evolver = RVEA(
        problem,
        n_iterations=it_learning+it_decision,
        n_gen_per_iter=GENS,
        population_size=POP,
        interact=True,
...
#    main(1254,"DTLZ2",3,100,"SBX",0.5,"bounds",10,0.2,"polynomial",0.5,"bounds",20,0.5,"random",None)
...
if __name__ == "__main__":
    ...
    main(args.seed, args.prob, args.obj, args.gens, args.cros,args.cros_prob, args.cros_rep, args.cros_dist, args.cros_alpha, args.mut, args.mut_prob, args.mut_repair, args.mut_pmd, args.mut_ump, args.sel, args.sel_size)
